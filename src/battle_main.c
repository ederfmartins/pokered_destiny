...
EWRAM_DATA u8 gDamageTypeForMove = 0;
...
static u8 CreateNPCTrainerParty(struct Pokemon *party, u16 trainerNum)
{
     ...
     && !(gBattleTypeFlags & (BATTLE_TYPE_BATTLE_TOWER | BATTLE_TYPE_EREADER_TRAINER | BATTLE_TYPE_TRAINER_TOWER)))
    {
        ...
        for (i = 0; i < gTrainers[trainerNum].partySize; i++)
        {
            ...
            {
                const struct TrainerMonNoItemCustomMovesAbility *partyData = gTrainers[trainerNum].party.NoItemCustomMovesAbility;
                u8 abilityNum;
             ...
             || gBattleCommunication[GetBattlerAtPosition(BATTLE_PARTNER(position))] == STATE_WAIT_ACTION_CONFIRMED)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        BtlController_EmitChooseAction(0, gChosenActionByBattler[gActiveBattler], gBattleBufferB[0][1] | (gBattleBufferB[0][2] << 8));
                        MarkBattlerForControllerExec(gActiveBattler);
